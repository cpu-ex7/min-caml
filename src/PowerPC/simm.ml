  ...
  | Let((x, t), Li(i), e) when -32768 <= i && i < 32768 ->
    ...
    if List.mem x (fv e') then Let((x, t), Li(i), e') else
      ((* Format.eprintf "erased redundant Set to %s@." x; *)
        e')
  | Let(xt, Slw(y, C(i)), e) when Env.mem y env -> (* for array access *)
    (* Format.eprintf "erased redundant Slw on %s@." x; *)
    g env (Let(xt, Li((Env.find y env) lsl i), e))
...
and g' env = function (* 各命令の16bit即値最適化 (caml2html: simm13_gprime) *)
  ...
  | Slw(x, V(y)) when Env.mem y env -> Slw(x, C(Env.find y env))
  | Lwz(x, V(y)) when Env.mem y env -> Lwz(x, C(Env.find y env))
  | Stw(x, y, V(z)) when Env.mem z env -> Stw(x, y, C(Env.find z env))
  | Lfd(x, V(y)) when Env.mem y env -> Lfd(x, C(Env.find y env))
  | Stfd(x, y, V(z)) when Env.mem z env -> Stfd(x, y, C(Env.find z env))
