...
let rec g env = function (* 命令列の13bit即値最適化 (caml2html: simm13_g) *)
  ...
  | Let(xt, SLL(y, C(i)), e) when Env.mem y env -> (* for array access *)
    ...
    g env (Let(xt, Set((Env.find y env) lsl i), e))
...
and g' env = function (* 各命令の13bit即値最適化 (caml2html: simm13_gprime) *)
  ...
  | Ld(x, V(y)) when Env.mem y env -> Ld(x, C(Env.find y env))
  | St(x, y, V(z)) when Env.mem z env -> St(x, y, C(Env.find z env))
  | LdDF(x, V(y)) when Env.mem y env -> LdDF(x, C(Env.find y env))
  | StDF(x, y, V(z)) when Env.mem z env -> StDF(x, y, C(Env.find z env))
