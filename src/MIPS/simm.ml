...
and g' env = function (* 各命令の16bit即値最適化 (caml2html: simm13_gprime) *)
  ...
  | Mul(x, V(y)) when Env.mem y env -> Mul(x, C(Env.find y env))
  | Mul(x, V(y)) when Env.mem x env -> Mul(y, C(Env.find x env))
  | Div(x, V(y)) when Env.mem y env -> Div(x, C(Env.find y env))
  | Sll(x, V(y)) when Env.mem y env -> Sll(x, C(Env.find y env))
  | Srl(x, V(y)) when Env.mem y env -> Srl(x, C(Env.find y env))
