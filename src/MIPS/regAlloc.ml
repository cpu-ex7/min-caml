  ...
  if is_reg x then Alloc(x) else
    ...
    try
      ...
      let live = (* 生きているレジスタ *)
        List.fold_left
          (fun live y ->
             if is_reg y then IdSet.add y live else
               try IdSet.add (Env.find y regenv) live
               with Not_found -> live)
          IdSet.empty
          free in
      let r = (* そうでないレジスタを探す *)
        List.find
          (fun r -> not (IdSet.mem r live))
          (prefer @ all) in
    ...
    with Not_found ->
      ...
      let y = (* 型の合うレジスタ変数を探す *)
        List.find
          (fun y ->
             not (is_reg y) &&
             try List.mem (Env.find y regenv) all
             with Not_found -> false)
          (List.rev free) in
      Format.eprintf "spilling %s from %s@." y (Env.find y regenv);
      Spill(y)
...
let rec g dest cont regenv = function (* 命令列のレジスタ割り当て (caml2html: regalloc_g) *)
  ...
  | Let((x, t) as xt, exp, e) ->
    ...
    (match alloc dest cont' regenv1 x t with
     | Spill(y) ->
       let r = Env.find y regenv1 in
       let (e2', regenv2) = g dest cont (add x r (Env.remove y regenv1)) e in
       let save =
         try Save(Env.find y regenv, y)
...
and g' dest cont regenv = function (* 各命令のレジスタ割り当て (caml2html: regalloc_gprime) *)
  | Nop | LoadImmediate _ | LoadAddres _ | Comment _ | Restore _ | FLoadImmediate _ | Read | FRead as exp -> (Ans(exp), regenv)
  ...
  | Load(x, y') -> (Ans(Load(find x Type.Int regenv, find' y' regenv)), regenv)
  | Store(x, y, z') -> (Ans(Store(find x Type.Int regenv, find y Type.Int regenv, find' z' regenv)), regenv)
...
and g'_if dest cont regenv exp constr e1 e2 = (* ifのレジスタ割り当て (caml2html: regalloc_if) *)
      ...
      (fun regenv' x ->
         try
           if is_reg x then regenv' else
             let r1 = Env.find x regenv1 in
             let r2 = Env.find x regenv2 in
  ...
  (List.fold_left
     (fun e x ->
        if x = fst dest || not (Env.mem x regenv) || Env.mem x regenv' then e else
          seq(Save(Env.find x regenv, x), e)) (* そうでない変数は分岐直前にセーブ *)
     (Ans(constr e1' e2'))
     (fv cont),
   regenv')
and g'_call dest cont regenv exp constr ys zs = (* 関数呼び出しのレジスタ割り当て (caml2html: regalloc_call) *)
  (List.fold_left
     (fun e x ->
        if x = fst dest || not (Env.mem x regenv) then e else
          seq(Save(Env.find x regenv, x), e))
     (Ans(constr
...
let h { name = Id.L(x); args = ys; fargs = zs; body = e; ret = t } = (* 関数のレジスタ割り当て (caml2html: regalloc_h) *)
  let regenv = Env.add x reg_cl Env.empty in
...
let f (Prog(data, fundefs, e)) = (* プログラム全体のレジスタ割り当て (caml2html: regalloc_f) *)
  ...
  let e', regenv' = g (Id.gentmp Type.Unit, Type.Unit) (Ans(Nop)) Env.empty e in
  Prog(data, fundefs', e')
